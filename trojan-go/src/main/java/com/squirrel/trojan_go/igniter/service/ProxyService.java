package com.squirrel.trojan_go.igniter.service;
...
import com.squirrel.trojan_go.igniter.connection.TestConnection;
import com.squirrel.trojan_go.igniter.connection.TrojanConnection;
import com.squirrel.trojan_go.igniter.helper.ClashHelper;
import com.squirrel.trojan_go.igniter.helper.JNIHelper;
 ...
 * {@link TrojanConnection}. For further information, see
 * {@link TrojanConnection}.
 */
public class ProxyService extends VpnService implements TestConnection.OnResultListener {
    ...
    private void startForegroundNotification(String channelId) {
        Intent openMainActivityIntent = new Intent(this, ProxyService.class);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Set<String> exemptAppPackageNames = new HashSet<>();    // getExemptAppPackageNames();   Update By Wendell
        ...
        TrojanHelper.ChangeListenPort(GlobalConfig.getTrojanConfigPath(), trojanPort);
        TrojanHelper.ShowConfig(GlobalConfig.getTrojanConfigPath());

        JNIHelper.trojan(GlobalConfig.getTrojanConfigPath());
        ...
        if (enable_clash) {
            try {
                ...
                ClashHelper.ChangeClashConfig(GlobalConfig.getClashConfigPath(),
                        trojanPort, clashSocksPort);
                ClashHelper.ShowConfig(GlobalConfig.getClashConfigPath());
